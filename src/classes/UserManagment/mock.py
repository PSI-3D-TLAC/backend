"""Minimal mock for user management used by the test backend.

Hardcoded users for each role and a trivial token store so the frontend can
exercise the login/logout flow without real authentication.
"""
...
# role -> sample user
USERS: List[dict] = [
    # Primary test accounts (per issue spec, password "1234").
    ...
    # Extra demo accounts (kept for completeness, password "test").
